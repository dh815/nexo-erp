...
const emptyForm = { descricao: '', categoria: '', valor: '', fornecedor: '', data: '', formaPagamento: '', observacoes: '' };
const formasPagamento = ['Pix', 'Cartão', 'Boleto', 'Dinheiro', 'Transferência'];

export default function Saidas() {
  const { data: saidas, loading, add, update, remove } = useCollection('saidas');
  ...
  const columns = [
    ...
    { key: 'categoria', header: 'Categoria', render: (r) => r.categoria ? <Pill color="orange">{r.categoria}</Pill> : '—' },
    { key: 'fornecedor', header: 'Fornecedor', render: (r) => <span className="text-muted">{r.fornecedor || '—'}</span> },
    { key: 'valor', header: 'Valor', render: (r) => <span className="font-semibold text-danger">- {money(r.valor)}</span> },
    { key: 'data', header: 'Data', render: (r) => <span className="text-muted">{r.data ? new Date(r.data).toLocaleDateString('pt-BR') : '—'}</span> },
  ];

  return (
    <div>
      <PageHeader crumb="Financeiro" title="Saídas" subtitle="Controle despesas, fornecedores e formas de pagamento" actionLabel="Nova saída" onAction={openNew} />

      <div className="bg-white border border-line rounded-2xl shadow-card p-5">
        <DataTable columns={columns} rows={saidas} loading={loading} onEdit={openEdit} onDelete={(r) => remove(r.id)}
          emptyMessage="Nenhuma saída registrada ainda." />
      </div>

      {modal && (
        <Modal title={modal.id ? 'Editar saída' : 'Nova saída'} onClose={() => setModal(null)}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Categoria"><input className={inputClass} value={form.categoria} onChange={(e) => set('categoria', e.target.value)} placeholder="Ex: Estoque" /></Field>
